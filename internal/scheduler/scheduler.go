...
func ejecutarAlertsVencimientos() {
	...
	for rows.Next() {
		...
		empRows, _ := compDB.Query(q(`
			SELECT nombre, codigo_empleado, fecha_vencimiento
			FROM empleados
			WHERE activo=1
			  AND fecha_vencimiento != ''
			  AND fecha_vencimiento <= $1`), en30)
		if empRows == nil {
			continue
		}
		...
		emailRows, _ := adminDB.Query(
			q("SELECT email FROM usuarios WHERE compania_id=$1 AND activo=1 AND rol IN ('admin','gerente')"),
			compID)
		var emails []string
		if emailRows != nil {
...
func ejecutarReporteSemanal() {
	...
	for rows.Next() {
		...
		compDB.QueryRow(q(`SELECT COUNT(*) FROM registros WHERE timestamp >= $1 AND timestamp <= $2`),
			desde+"T00:00:00", hasta+"T23:59:59").Scan(&regSemana)

		// Tardanzas: entradas después de las 8:15 (hora + tolerancia 15min)
		compDB.QueryRow(q(`SELECT COUNT(*) FROM registros
			WHERE tipo='entrada' AND timestamp >= $1 AND timestamp <= $2
			  AND strftime('%H:%M', timestamp) > '08:15'`),
			desde+"T00:00:00", hasta+"T23:59:59").Scan(&tardanzas)

		emailRows, _ := adminDB.Query(
			q("SELECT email FROM usuarios WHERE compania_id=$1 AND activo=1 AND rol IN ('admin')"),
			compID)
		if emailRows == nil {
			continue
		}
