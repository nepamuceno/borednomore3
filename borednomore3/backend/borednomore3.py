...
OPTIONS:
    ...
    --loop                      Analysis mode: Play the current transition in a loop
...
def main():
    ...
    parser.add_argument('--loop', action='store_true', help="Play transition in loop for analysis")
    ...
    if args.loop:
        logger.info("LOOP MODE ACTIVE: Use Ctrl+C to move to the next transition.")
        ...
        while True:
            ...
            if args.loop:
                try:
                    ...
                    logger.info("Loop interrupted. Moving to next wallpaper...")
            else:
