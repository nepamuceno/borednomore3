...
of timing values."""
	...
	def show_help(self):
	    ...
	    if os.path.exists(script):
	        if script.endswith('.py'):
	            ...
	            self.run_command_and_show_output([script, '-h'], "HELP OUTPUT")
	    else:
	...
	def show_version(self):
	    ...
	    if os.path.exists(script):
	        if script.endswith('.py'):
	            ...
	            self.run_command_and_show_output([script, '--version'], "VERSION INFO")
	    else:
	        messagebox.showerror("Error", f"Script not found:\n{script}")
	...
	def show_credits(self):
	    ...
	    if os.path.exists(script):
	        if script.endswith('.py'):
	            ...
	            self.run_command_and_show_output([script, '-c'], "CREDITS")
	    else:
	        messagebox.showerror("Error", f"Script not found:\n{script}")
	...
	def build_command(self):
	    script = self.script_path_var.get()
	    # If script is a wrapper in /usr/bin, call it directly without python3
	    if script in ['/usr/bin/borednomore3', '/usr/local/bin/borednomore3'] or not script.endswith('.py'):
	        cmd = [script]
	    else:
	        cmd = [self.python_var.get(), script]
